...
class Event extends Component {
  ...
  renderRating = (totalSongs = 1, score = 0, el = "h1", emoji = "no") => {
    ...
    return <Rating emoji={emoji} count={count} el={el} />;
  };
              ...
              <Grid item xs={12}>
                <Grid container direction="row" justify="space-around">
                  <Grid className="info" key={1} item>
                    ...
                    {/* // TODO show date */}
                    <h3>
                      {data.event.venue.name}, {data.event.venue.city.name},{" "}
                      {data.event.venue.city.state}
                    </h3>
                    <h2>
                      {moment(data.event.eventDate, "DD-MM-YYYY").format(
                        "MMM DD YYYY"
                      )}
                    </h2>
